describe('THEMMOI_BN_29', function(){
	context('Kiem tra khi du lieu la so co chu so 0 o dau', function(){
		...
		it('Kiem tra truong so dien thoai', function(){
			...
			cy.get("input[autofocus='']").type("my dfvdog")
			...
			cy.get("input[name='mobile_phone']").type("                  08375947598                         ")
			...
			cy.get("[ng-submit='form.$valid && createPatient()'] [ng-model='patient.identification_number']").type("16709889")
			...
			cy.get("[ng-submit='form.$valid && createPatient()'] [ng-model='contact.name']").type("ldgjsdffad")
