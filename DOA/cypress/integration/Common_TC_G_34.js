describe('Common TC_GUI', function() {
  context('Validate du lieu truong so', function() {
    beforeEach(function() {
     ...
     it('Nhap du lieu la so thap phan(sua thong tin benh nhan)', function(){
      cy
        ...
        .get('a[href$="/main/patients/858/edit" ]').first().click()
        .get("input[name='mobile_phone']").clear()
        .get("input[name='mobile_phone']").type("10.343")
        .get("[ng-submit='form.$valid && updatePatient()'] [ng-model='patient.identification_number']").clear()
        .get("[ng-submit='form.$valid && updatePatient()'] [ng-model='patient.identification_number']").type("10.3231")
      ...
      // da focus vao phan tu co thuoc tinh nam=mobile_phone
        ...
        .focused().should('have.attr','ng-model','patient.identification_number')
    })

     it('Nhap du lieu la so thap phan(Them moi don thuoc)',function(){
      cy
        .get("input[name='email']").type(Cypress.env('USER_DOCTOR'))
        .get("input[name='password']").type(Cypress.env('LOGIN_PASSWORD'))
        .get("button[type='submit']").click()
        ...
        .contains("Tạo đơn thuốc thành công")

    })
     it('Nhap du lieu la so thap phan(sua thong tin benh nhan)_taikhoanadmin', function(){
      ...
      cy.get('a[href$="/main/patients/860/edit" ]').second().click()
      cy.get("input[name='mobile_phone']").clear()
      cy.get("input[name='mobile_phone']").type("4.567")
      cy.get("[ng-submit='form.$valid && updatePatient()'] [ng-model='patient.identification_number']").clear()
      cy.get("[ng-submit='form.$valid && updatePatient()'] [ng-model='patient.identification_number']").type("7.654")
      cy.get('[ng-submit="form.$valid && updatePatient()"] [type="submit"]').click()
      cy.contains("không đúng định dạng")
      // da focus vao phan tu co thuoc tinh nam=mobile_phone
      cy.focused().should('have.attr', 'name', 'mobile_phone')
      cy.focused().should('have.attr','ng-model','patient.identification_number')
    })

     it('Nhap du lieu la so thap phan (them moi don thuoc)_taikhoanadmin', function(){
      cy
      ...
      it('Nhap du lieu la so thap phan (them moi benh nhan)_taikhoanbacsi', function(){
        ...
        cy.get("[ng-submit='form.$valid && createPatient()'] [ng-model='patient.identification_number']").type("56.445")
    ...
    it('Nhap du lieu la so thap phan (them moi benh nhan)_taikhoanadmin', function(){
        cy.get("input[name='email']").type(Cypress.env('USER_AGENCY'))
        cy.get("input[name='password']").type(Cypress.env('LOGIN_PASSWORD'))
        cy.get("button[type='submit']").click()
        cy.wait(5000)
