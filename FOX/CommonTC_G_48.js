	...
	describe('Tài khoản trưởng cơ sở', function () {
		...
		describe('Quản lý bệnh nhân', function () {
			...
			it('Textarea Thêm đơn thuốc > Y lệnh', function () {
				...
				cy.contains('div.form-group', 'Y lệnh').find('textarea').type(STRING_VERY_LONG);

				cy.contains('div.modal-footer button', 'Lưu').click().wait(1000).should('exist');
				cy.contains('div.form-group', 'Y lệnh').find('textarea').should('exist');
			});
		});
	});
