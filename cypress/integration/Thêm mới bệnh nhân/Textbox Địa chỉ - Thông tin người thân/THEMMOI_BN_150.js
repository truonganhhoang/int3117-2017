...
describe(`Kiem tra Combo Box voi tai khoan ${agency}`, function(){
    ...
    it("Click Địa chỉ", function(){
        cy.wait(delay)
        cy.get('[ng-model="contact.address"]').prev().prev().click()
    })
    it("So sanh", function(){
        cy.wait(delay)
        const str_default = Cypress.$("[name='resident_district_id'] a span span").text().trim() + ' - ' + Cypress.$("[name='resident_province_id'] a span span").text().trim();
