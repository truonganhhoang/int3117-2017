...
describe(`Kiem tra Combo Box voi tai khoan ${agency}`, function(){
    ...
    it("Kiểm tra là trường không bắt buộc", function(){
        ...
        cy.get('[name="name"]').eq(0).type('Nguyen Van ade');

        cy.get('[name="birth_date"]').type('11/01/2011').type('{esc}');
        cy.get("label").contains("Giới tính")
            .next().find('a.select2-choice').click()

        cy.get('#ui-select-choices-row-13-1').click();

        cy.get('[name="mobile_phone"]').type('0988868686');
        
        cy.get("label").contains("Nghề nghiệp")
            .next().find('a.select2-choice').click()
        cy.get('#ui-select-choices-row-15-0').click();
        ...
        cy.get("label").contains("Trình độ học vấn")
            .next().find('a.select2-choice').click()
        cy.get('#ui-select-choices-row-17-0').click();

        cy.get("label").contains("Tình hình tài chính")
            .next().find('a.select2-choice').click()
        cy.get('#ui-select-choices-row-18-0').click();

        cy.get('[name="admission_date"]').type('14/09/2017').type('{esc}');
        cy.get("label").contains("Nơi giới thiệu")
            .next().type('DEF')

        cy.get("label").contains("Tỉnh/Thành phố thường trú")
            .next().find('a.select2-choice').click()
        cy.get('#ui-select-choices-row-19-1').click();
        
        cy.get("label").contains("Huyện/Quận thường trú")
            .next().find('a.select2-choice').click()
        cy.get('#ui-select-choices-row-20-0').click();
        ...
        cy.get("label").contains("Xã/Thị Trấn tạm trú")
            .next().find('a.select2-choice').click()
        cy.get('#ui-select-choices-row-24-0').click();
...
describe(`Kiem tra Combo Box voi tai khoan ${doctor}`, function(){
    ...
    it("kKiểm tra là trường không bắt buộc", function(){
        cy.wait(delay)

        cy.get('[name="name"]').eq(0).type('Nguyen Van cde');

        cy.get('[name="birth_date"]').type('25/01/2011').type('{esc}');
        cy.get("label").contains("Giới tính")
            .next().find('a.select2-choice').click()
        ...
        cy.get("label").contains("Trình độ học vấn")
            .next().find('a.select2-choice').click()
        cy.get('#ui-select-choices-row-17-1').click();

        cy.get("label").contains("Tình hình tài chính")
            .next().find('a.select2-choice').click()
        cy.get('#ui-select-choices-row-18-1').click();

        cy.get('[name="admission_date"]').type('15/09/2017').type('{esc}');
        cy.get("label").contains("Nơi giới thiệu")
            .next().type('DEF')

        cy.get("label").contains("Tỉnh/Thành phố thường trú")
            .next().find('a.select2-choice').click()
        cy.get('#ui-select-choices-row-19-3').click();
