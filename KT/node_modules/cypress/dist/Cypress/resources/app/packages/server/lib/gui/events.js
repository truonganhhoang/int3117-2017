(function() {
  var Project, Updater, Windows, _, api, dialog, errors, handleEvent, ipc, logger, logs, open, openProject, pgk, shell, user;
  ...
  handleEvent = function(options, bus, event, id, type, arg) {
    var onBus, onFocusTests, onSettingsChanged, onSpecChanged, onWarning, send, sendErr, sendNull, sendResponse;
    sendResponse = function(data) {
      if (data == null) {
        data = {};
      }
      try {
        logger.info("sending ipc data", {
          type: type,
          data: data
        });
    ...
    switch (type) {
      ...
      case "get:options":
        return pgk(options).then(send)["catch"](sendErr);
      case "updater:check":
