(function() {
  var Promise, Socket, _, automation, browsers, cwd, errors, exec, files, fixture, fs, isSpecialSpec, log, logger, open, path, pathHelpers, reporterEvents, retry, runnerEvents, socketIo, uuid,
    slice = [].slice;
  ...
  Socket = (function() {
    ...
    Socket.prototype.onTestFileChange = function(filePath) {
      logger.info("onTestFileChange", {
        filePath: filePath
      });
      if (!/\.(js|jsx|coffee|cjsx)$/.test(filePath)) {
        return;
      }
    ...
    Socket.prototype.watchTestFileByPath = function(config, originalFilePath, watchers, options) {
      var filePath;
      ...
      if ((filePath === this.testFilePath) || (config.watchForFileChanges === false)) {
        return;
      }
      logger.info("watching test file", {
        path: filePath
      });
      if (this.testFilePath) {
        watchers.removeBundle(this.testFilePath);
      }
      this.testFilePath = filePath;
      log("will watch test file path " + filePath);
      return watchers.watchBundle(filePath, config, {
        onChange: this.onTestFileChange.bind(this)
      })["catch"](function() {});
    };
    ...
    Socket.prototype.startListening = function(server, watchers, automation, config, options) {
      ...
      return this.io.on("connection", (function(_this) {
        return function(socket) {
          ...
          socket.on("watch:test:file", function(filePath, cb) {
            ...
            _this.watchTestFileByPath(config, filePath, watchers, options);
            return cb();
          });
